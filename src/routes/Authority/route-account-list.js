...
class AccountList extends Component {
  ...
  handleAdd = (title, record) => {
    const newRecord = { ...record, roleIds: ['1', '2'] };
    this.setState({
      modalTitle: title,
      modalVisible: true,
  ...
  handleCancel = () => {
    this.setState({
      modalVisible: false,
      eidtAccount: { createTime: moment().format('YYYY-MM-DD') },
    });
  };
  ...
  render() {
    ...
    const columns = [
      ...
      { title: '所属部门', dataIndex: 'department', key: 'source', align: 'center' },
